...
/*   Updated: 2023/07/28 15:05:29 by bsirikam         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char	*findpath(char *prog_name, t_data *g_data)
{
	char	**split_path;
	char	*sl_progname;
	char	*prog2exe;
	...
	split_path = ft_split(my_getenv("PATH", g_data), ':');
	if (!split_path)
	{
		free(sl_progname);
		err_msgexec(NULL, strerror(errno));

...
/*   Updated: 2023/07/28 15:48:11 by bsirikam         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char	*sub_expand_proc(char *str, t_c *c, char *fin, t_data *g_data)
{
	char	*name;
	char	*var;
	char	*varfree;
...
static char	*subinsub_expand(char *fin, t_c *c, t_strcut *cur, t_data *g_data)
{
	if (c->st != c->i)
		fin = ssp_strnjoin(fin, cur->str + c->st, (c->i - c->st), 0);
	if (!fin)
		return (fin);
	fin = sub_expand_proc(cur->str, c, fin, g_data);
	if (!fin)
		return (fin);
	c->st = (c->i)--;
	return (fin);
...
// pass 0 to both int
static void	sub_expand(t_strcut *cur, t_data *g_data)
{
	char	*fin;
	t_c		*c;
	...
	while (cur->str[c->i])
	{
		if (cur->str[c->i] == '$')
		{
			fin = subinsub_expand(fin, c, cur, g_data);
			if (!fin)
				return (set_error(cur));
		}
		c->i++;
...
void	expand_var(t_strcut *head, t_data *g_data)
{
	t_strcut	*cur;
